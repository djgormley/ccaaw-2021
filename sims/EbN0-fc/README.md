...
Literal zero is not in the script's FCW bin vector; the grid's displayed 0-MHz point corresponds to FCW `-8`. The one omitted record at 3 dB and 33 at 0 dB leave row masses equal to the corresponding reported symbol-rate detection proportions, which is consistent with literal zero representing no returned center-frequency estimate. No retained generator or driver linked to these five files supplies the random seed, true center frequency, or acceptance tolerance, so this interpretation is not independently proven and the files cannot establish calibrated center-frequency accuracy.

Run `python3 plot_characterization.py` from any directory to regenerate `images/EbN0_fc_histo.png`. The corrected figure uses marker area for per-bin trial proportion and places unmatched literal zeros in a separate `literal FCW 0` category, so all 300 records in every row remain visible without interpreting those zeros as frequency estimates.
